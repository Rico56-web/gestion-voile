...
page_stats.py
==============
Tableau de bord statistiques et financier.

Contenu de la page :
- Indicateurs clés (cliquer sur un pavé affiche son tableau détaillé).
- Répartition par société.
- Graphique chronologique + seuil de rentabilité.
- Alerte de vidange moteur.

Sources de données :
- croisieres_v2.json / etapes_v2.json / contacts_v2.json
- maintenance.json (lu tel quel, sans migration)
- params.json (charges fixes de référence, seuil de vidange)

Pas encore disponible : un éditeur pour modifier les charges fixes de
référence ("frais_fixes") depuis l'appli. Pour l'instant elles se
changent en éditant params.json directement.
"""
...
def afficher_page_stats(charger_croisieres, charger_etapes, charger_maintenance, charger_params, charger_contacts):
    ...
    st.caption("📌 Les charges fixes de référence se modifient pour l'instant dans params.json (éditeur à venir).")
