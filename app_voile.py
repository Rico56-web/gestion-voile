...
if st.session_state.page == "LISTE":
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt').iterrows():
            ...
            st.markdown(f"""<div class="client-card" style="border-left:12px solid {col_s};">
                ...
                📧 <a href="mailto:{r.get('Email','')}" style="color:#1a2a6c;text-decoration:none;">{r.get('Email','')}</a> | 📞 <a href="tel:{tel_c}" style="color:#1a2a6c;text-decoration:none;font-weight:bold;">{r.get('Téléphone','')}</a><br>
                <a href="https://wa.me/{tel_c}" target="_blank" class="wa-btn">💬 WA</a><br>
    ...
    if not df_cmn.empty:
        ...
        st.text_area("Aperçu", corps, height=120)
    ...
    else: st.info("Aucune prestation CMN ce mois.")
    ...
    if st.session_state.edit_f_idx is not None:
        ...
        st.button("➕ FRAIS", on_click=edit_frais, args=("NEW",), use_container_width=True)
