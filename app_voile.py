...
if st.session_state.page == "LISTE":
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt').iterrows():
            ...
            # 4. BOUTONS D'ACTION (Crayon et Poubelle)
            ce, cd = st.columns([1, 2])
            ce.button("✏️ Modifier", key=f"nav_e_{i}", on_click=edit_nav, args=(i,))
            
            if cd.checkbox("🗑️ Supprimer", key=f"nav_c_{i}"):
                if st.button("Confirmer suppression", key=f"nav_b_{i}"):
                    df = df.drop(i)
