...
# --- 6. PAGE PLANNING ---
        ...
        for _, r in df_c.iterrows():
            try:
                ...
                if mv == sel_m and yv == sel_y:
                    ...
                    # On définit la couleur de CETTE ligne précise
                    ...
                    for j in range(dv, dv + n_j):
                        if j in jours_occ:
                            ...
                            # RÈGLE : Le ROUGE gagne toujours sur les autres couleurs
                            if current_c == "#e74c3c" or existing_c == "#e74c3c":
                                jours_occ[j]["c"] = "#e74c3c"
                            # Le VERT gagne sur le bleu/jaune pour signaler une mission active
                    ...
                    # On ajoute au détail pour la liste sous le calendrier
        ...
        # --- LE CALENDRIER SVG (FORMAT IPHONE) ---
        ...
        for sem in cal_mat:
            ...
            for jour in sem:
                if jour == 0:
                    h_cal += '<td style="height: 42px; border: 0.5px solid #eee;"></td>'
                else:
