...
if st.session_state.page == "LISTE":
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt').iterrows():
            ...
            # 2. Gestion du prix et de l'alerte
            prix_val = to_f(r.get("PrixJour", 0))
            prix_str = fmt_p(prix_val)
            alerte_prix = ""
            if prix_val <= 0:
                alerte_prix = '<div style="color: #e74c3c; font-weight: bold; font-size: 0.85rem; margin-top: 5px; border: 1px dashed #e74c3c; padding: 2px; text-align: center;">⚠️ PRIX NON SAISI !</div>'
            
            # 3. Couleur : Bleu si CMN, sinon selon statut
            if soc.upper() == "CMN":
                col_s = "#3498db"
            else:
    ...
    if not df.empty:
        for i, r in df.iterrows():
            ...
            if d_s.year == p_y and d_s.month == p_m:
                ...
                cl = "day-cmn" if s_u == "CMN" else ("day-ok" if "OK" in str(r.get('Statut','')).upper() or "🟢" in str(r.get('Statut','')) else "day-wait")
    ...
    st.markdown("<style>td, th { white-space: nowrap !important; font-size: 0.8rem !important; }</style>", unsafe_allow_html=True)
