...
# --- MENU PRINCIPAL (AVEC BOUTON VERT) ---
...
if st.session_state.page == "LISTE":
    ...
    if not df.empty:
        ...
        for i, r in data.iterrows():
            ...
            if c_del.button("🗑️ Effacer", key=f"del_{i}"):
...
# 2. PAGE PLANNING
    ...
    sel_y = y_col.selectbox("Année", [2025, 2026], index=1)
    sel_m = m_col.selectbox("Mois", range(1, 13), index=datetime.now().month-1)
    ...
    if not df.empty:
        for i, r in df.iterrows():
            ...
            if d_start.year == sel_y and d_start.month == sel_m:
                color = "day-ok" if "OK" in str(r.get('Statut')).upper() or "🟢" in str(r.get('Statut')) else "day-wait"
...
# 3. PAGE STATS (INCHANGÉE COMME DEMANDÉ)
...
# 4. PAGE MAINTENANCE (AVEC BOUTON MODIFIER CRAYON)
elif st.session_state.page == "FRAIS":
    st.markdown('<div class="page-title">🔧 MAINTENANCE</div>', unsafe_allow_html=True)
    idx_f = st.session_state.edit_f_idx
    with st.expander("📝 SAISIR / MODIFIER UN FRAIS", expanded=(idx_f is not None)):
