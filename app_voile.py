...
menu = [
    ...
    ("📖 LIVRE","LOGBOOK"), ("📄 FACT","FACTURE"), ("🛟 SÉCU","SECU"), 
...
if st.session_state.page == "LISTE":
    ...
    search_query = st.text_input("🔍 Recherche par nom...", "").lower()
    c1, c2 = st.columns(2)
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt', ascending=(st.session_state.view_mode=="FUTURES")).iterrows():
            ...
            st.markdown(f"""<div class="client-card" style="border-left: 10px solid {"#3498db" if soc=="CMN" else "#ccc"};">
                <div class="status-badge">{r.get('Statut','🟡')}</div>
    ...
    with st.form("f_livre_bord"):
        ...
        if st.form_submit_button("⚓ ENREGISTRER L'ÉTAPE", use_container_width=True):
            dist, h_mot = a_mi - d_mi, a_h - d_h
            row = {"Date":date_l, "LieuD":d_lieu, "HeuresD":d_h, "MillesD":d_mi, "Obs":obs, "LieuA":a_lieu, "HeuresA":a_h, "MillesA":a_mi, "Dist":dist, "DeltaH":h_mot}
    ...
    if idx_log is not None and st.button("❌ Annuler"): st.session_state.edit_log_idx = None; st.rerun()
    ...
    if not df_log.empty:
        ...
        for i in reversed(df_log.index):
            ...
            with st.expander(f"📅 {l['Date']} | {l['LieuD']} ➔ {l['LieuA']}"):
                ...
                b1.metric("DISTANCE PARCOURUE", f"{float(l.get('Dist',0)):.1f} MN")
                b2.metric("TEMPS MOTEUR", f"{float(l.get('DeltaH',0)):.1f} H")
                b3.metric("INDEX LOCH FIN", f"{float(l.get('MillesA',0)):.0f}")
                if l.get('Obs'): st.info(f"📝 {l['Obs']}")
                ce, cd = st.columns(2)
                if ce.button("✏️ Modifier", key=f"ed_log_{i}"): st.session_state.edit_log_idx = i; st.rerun()
                if cd.button("🗑️ Supprimer", key=f"del_log_{i}"): df_log = df_log.drop(i); sauvegarder_data(df_log, "livre_de_bord.json"); st.rerun()

elif st.session_state.page == "PLANNING":
    st.markdown('<div class="page-title">🗓️ PLANNING</div>', unsafe_allow_html=True)
    y_col, m_col = st.columns(2)
    p_y, p_m = y_col.selectbox("Année", [2025, 2026, 2027], index=1), m_col.selectbox("Mois", range(1, 13), index=datetime.now().month-1)
    occu = {}
    if not df.empty:
        for _, r in df.iterrows():
            if "🔴" not in str(r.get('Statut','')):
                d_s, nb_j = parse_d(r['DateNav']), int(to_f(r.get('NbJours', 1)))
                for j in range(nb_j):
                    curr = d_s + timedelta(days=j)
                    if curr.year == p_y and curr.month == p_m:
                        occu[curr.day] = "day-cmn" if str(r.get('Société','')).upper() == "CMN" else "day-ok"
    ...
    if not df.empty:
        df['dt'] = df['DateNav'].apply(parse_d)
        t_rev = sum(df[(df['dt'].dt.year == s_y) & (df['Statut'].str.contains("OK|🟢", na=False))]['PrixJour'].apply(to_f))
    if not df_f.empty:
        df_f['dt'] = df_f['Date'].apply(parse_d)
        t_fra = sum(df_f[df_f['dt'].dt.year == s_y]['Montant'].apply(to_f))
    
    c1, c2, c3 = st.columns(3)
    c1.metric("REVENUS (OK)", fmt_p(t_rev))
    c2.metric("FRAIS", fmt_p(t_fra))
    c3.metric("NET", fmt_p(t_rev - t_fra))
    st.progress(min(t_rev/st.session_state.cible_annuelle, 1.0))

elif st.session_state.page == "FACTURE":
    st.markdown('<div class="page-title">📄 FACTURATION CMN</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    f_y, f_m = c1.selectbox("Année", [2025, 2026, 2027], index=1), c2.selectbox("Mois", range(1, 13), index=datetime.now().month-1, format_func=lambda x: calendar.month_name[x])
    df_c = pd.DataFrame()
    if not df.empty:
        df['dt'] = df['DateNav'].apply(parse_d)
    ...
    if not df_c.empty:
        ...
        dest, sujet = "tresorier@cmn-asso.fr", f"Facturation Skipper {lbl}"
        gmail_url = f"https://mail.google.com/mail/?view=cm&fs=1&to={dest}&cc=eric.clavreul@gmail.com&sujet={urllib.parse.quote(sujet)}&body={urllib.parse.quote(corps)}"
        st.markdown(f'<a href="{gmail_url}" target="_blank" style="background-color:#db4437;color:white;padding:12px;display:block;text-align:center;text-decoration:none;border-radius:8px;font-weight:bold;">💻 ENVOYER VIA GMAIL</a>', unsafe_allow_html=True)
    ...
    if not df_arch.empty:
        ...
        for i in reversed(df_arch.index):
            ...
            with st.expander(f"{a['Mois']} — {fmt_p(a['Montant'])} ({"✅" if is_p else "⏳"})"):
                if st.button("Basculer statut Payé", key=f"p_{i}"): a["Payée"] = not is_p; sauvegarder_data(df_arch, "archives_factures.json"); st.rerun()
                if st.button("Supprimer l'archive", key=f"d_{i}"): df_arch = df_arch.drop(i); sauvegarder_data(df_arch, "archives_factures.json"); st.rerun()
    ...
    if st.session_state.edit_s_idx is not None:
        ...
        with st.form("f_s"):
            new_v = st.text_input("Point de contrôle :", v_init)
    ...
    if st.session_state.edit_f_idx is not None:
        ...
        for i in reversed(df_f.index):
            ...
            if st.button("Supprimer", key=f"fd_{i}"): df_f = df_f.drop(i); sauvegarder_data(df_f, "frais.json"); st.rerun()
    ...
    if st.session_state.edit_n_idx is not None:
        ...
        for i in reversed(df_n.index):
            ...
            if st.button("Supprimer", key=f"nd_{i}"): df_n = df_n.drop(i); sauvegarder_data(df_n, "notes.json"); st.rerun()
    ...
    with st.form("f_client"):
        ...
        d, j = st.text_input("Date (DD/MM/YYYY)", init.get("DateNav","")), st.text_input("Nb Jours", str(init.get("NbJours","1")))
