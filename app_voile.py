...
if st.session_state.page == "ARCHIVES":
    ...
    with t3: st.dataframe(charger_data_safe('archives_logbook.json'), use_container_width=True) 
        # =================================================================
# --- 12. PAGE LIVRE DE BORD (LOG) - CALCULS ET NOMENCLATURE ---
# =================================================================
if st.session_state.page == "LOG":
    ...
    # 1. RÉCUPÉRATION DERNIERS COMPTEURS (Ligne 0 = Haut)
    last_h, last_m = 0.0, 0.0
    if not df_log.empty:
        try:
            r0 = df_log.iloc[0]
            # On cherche dans MotArr ou Compteur_Arr pour la compatibilité
            last_h = float(r0.get('MotArr', r0.get('Compteur_Arr', 0.0)))
            last_m = float(r0.get('MilArr', r0.get('Compteur_Mil', 0.0)))
        except: pass
            ...
            for _, row in edited_steps.iterrows():
                if row.get("Port"):
                    ...
                    nouvelles.append({
                        ...
                        "MotDep": h_d,
                        "MotArr": h_a,              # REMPLI ICI
                        "TotalMot": h_a - h_d,      # CALCUL DURÉE
                        "MilDep": m_d,
                        "MilArr": m_a,              # REMPLI ICI
    ...
    if not df_log.empty:
        ...
        with st.expander("📝 MODIFIER UNE LIGNE"):
            ...
            with st.form(f"form_mod_v2_{idx_m}"):
                st.write(f"Modification de la ligne {idx_m}")
                ed_port = st.text_input("Port", value=r.get('PortArr', ''))
                
                c_m1, c_m2 = st.columns(2)
                m_d = c_m1.number_input("Moteur Départ", value=float(r.get('MotDep', 0.0)))
                m_a = c_m2.number_input("Moteur Arrivée", value=float(r.get('MotArr', r.get('Compteur_Arr', 0.0))))
                
                c_mi1, c_mi2 = st.columns(2)
                mi_d = c_mi1.number_input("Milles Départ", value=float(r.get('MilDep', r.get('MillesDep', 0.0))))
                mi_a = c_mi2.number_input("Milles Arrivée", value=float(r.get('MilArr', r.get('Compteur_Mil', 0.0))))
                
                ed_voile = st.number_input("Heures Voile", value=float(r.get('H_Voile', 0.0)))
                ed_notes = st.text_area("Notes", value=r.get('Notes', ''))
                
                if st.form_submit_button("VALIDER LES CALCULS"):
                    df_log.at[idx_m, 'PortArr'] = ed_port
                    # MOTEUR
                    df_log.at[idx_m, 'MotDep'] = m_d
                    df_log.at[idx_m, 'MotArr'] = m_a
                    df_log.at[idx_m, 'TotalMot'] = m_a - m_d
                    # MILLES
                    df_log.at[idx_m, 'MilDep'] = mi_d
                    df_log.at[idx_m, 'MilArr'] = mi_a
                    df_log.at[idx_m, 'TotalMil'] = mi_a - mi_d
