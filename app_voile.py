...
st.markdown("""
    ...
    .section-header { background: #2c3e50; padding: 10px; border-radius: 8px; margin: 20px 0 10px 0; color: white; font-weight: bold; text-align: center; }
...
if st.session_state.page == "FORM":
    ...
    if st.button("🔙 Annuler / Retour"): st.session_state.page = "LISTE"; st.rerun()

elif st.session_state.page == "LISTE":
    c_search, c_add = st.columns([3, 1])
    search = c_search.text_input("🔍 Rechercher Nom ou Société", value="").upper()
    ...
    # Division stricte Futur / Passé
    futur = df_filtered[df_filtered['dt_obj'] >= auj].sort_values('dt_obj', ascending=True)
    archives = df_filtered[df_filtered['dt_obj'] < auj].sort_values('dt_obj', ascending=False)
    
    def afficher_liste(data, titre):
        if not data.empty:
            st.markdown(f'<div class="section-header">{titre}</div>', unsafe_allow_html=True)
            for i, r in data.iterrows():
                # Détermination du style (CMN prioritaire)
                cl_style = ""
                if "CMN" in str(r['Société']).upper(): cl_style = "cmn-style"
                elif "🟢" in str(r['Statut']): cl_style = "status-ok"
                elif "🟡" in str(r['Statut']): cl_style = "status-attente"
                
                st.markdown(f"""
                    <div class="client-card {cl_style}">
                        <div style="float:right; font-weight:bold; color:#2c3e50;">{r["PrixJour"]}€</div>
                        <span style="font-size:1.1rem;"><b>{r["Prénom"]} {r["Nom"]}</b></span><br>
                        ...
                        <small style="color:#7f8c8d;">📅 {r["DateNav"]} | 🚢 {r["Milles"]} NM | ⚙️ {r["HeuresMoteur"]}h</small>
                    </div>
                """, unsafe_allow_html=True)
                if st.button(f"✏️ Modifier {r['Prénom']}", key=f"btn_{i}", use_container_width=True):
                    st.session_state.edit_idx = i; st.session_state.page = "FORM"; st.rerun()
        elif search == "":
             st.write(f"Aucune fiche dans {titre.lower()}")

    afficher_liste(futur, "🚀 PROCHAINES SORTIES")
    st.markdown("<br>", unsafe_allow_html=True)
    afficher_liste(archives, "📂 ARCHIVES (SORTIES PASSÉES)")

elif st.session_state.page == "PLAN":
    # Le planning avec couleur CMN restaurée
    c1, c2 = st.columns(2)
    ...
    for w in cal:
        ...
        for d in w:
            if d == 0: h_c += '<td style="background:#f9f9f9;"></td>'
                ...
                txt = "black"
                if dat:
                    if any("CMN" in str(x['Société']).upper() for x in dat): bg = "#3498db"; txt = "white"
                    elif any("🟢" in str(x['Statut']) for x in dat): bg = "#2ecc71"; txt = "white"
                    else: bg = "#f1c40f"; txt = "black"
                h_c += f'<td style="background:{bg}; color:{txt};">{d}</td>'
        h_c += '</tr>'
    st.markdown(h_c + '</table>', unsafe_allow_html=True)

elif st.session_state.page == "BUDGET":
    # Stats avec tableau mensuel
    y_b = st.selectbox("Statistiques de l'année", [2026, 2027, 2028])
    df['dt'] = df['DateNav'].apply(parse_date)
    df_y = df[(df['dt'].dt.year == y_b) & (df['Statut'].str.contains("🟢"))]
    
    st.markdown("### 📊 Cumul Mensuel")
    ht = '<table class="cal-table"><tr><th>Mois</th><th>Jours</th><th>Milles</th><th>CA (€)</th></tr>'
    mois_fr = ["Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Déc"]
    for i, m in enumerate(mois_fr, 1):
    ...
    with st.form("add_f"):
        ...
        f_type = st.selectbox("Type", ["Moteur", "Carburant", "Entretien", "Divers"])
