...
st.markdown("""
    ...
    .client-card { background: white; padding: 12px; border-radius: 10px; margin-bottom: 8px; border: 1px solid #ddd; }
    ...
    .cal-table { width: 100%; border-collapse: collapse; table-layout: fixed; margin-bottom: 15px; }
    ...
    .day-detail { padding: 8px; border-radius: 6px; margin-bottom: 5px; font-size: 0.85rem; background: #fdfdfd; border: 1px solid #eee; border-left: 8px solid #ccc; }
    </style>
...
if st.session_state.page == "LISTE":
    ...
    if st.session_state.confirm_del is not None:
        ...
        st.error(f"⚠️ Supprimer définitivement la fiche de {df.loc[idx, 'Nom']} ?")
        c1, c2 = st.columns(2)
        if c1.button("✅ OUI, Supprimer"): df = df.drop(idx); sauvegarder_data(df); st.session_state.confirm_del = None; st.rerun()
        if c2.button("❌ NON"): st.session_state.confirm_del = None; st.rerun()
    ...
    if st.button("➕ CRÉER UNE NOUVELLE NAVIGATION", use_container_width=True): st.session_state.edit_idx = None; st.session_state.page = "FORM"; st.rerun()
    ...
    if not df.empty:
        ...
        for i, r in data.iterrows():
            ...
            # Carte détaillée
            st.markdown(f'''
                <div class="client-card" style="border-left: 15px solid {color};">
                    ...
                    <span>📅 {r.get("DateNav","")} ({r.get("NbJours","1")}j)</span><br>
                    <span style="color:{color}; font-weight:bold; font-size:0.8rem;">{st_text}</span>
                </div>
            ''', unsafe_allow_html=True)
            
            cx, cy = st.columns(2)
            if cx.button("✏️ Modifier / Gérer", key=f"ed_{i}"):
...
# --- AUTRES PAGES ---
    ...
    st.markdown('<div class="page-title">💰 BILAN DÉTAILLÉ</div>', unsafe_allow_html=True)
    annee_choisie = st.selectbox("Année", [2026, 2027, 2028])
    # ... code stats restauré plus haut ...
    ...
    for m in range(1, 13):
        ...
        stats_mois.append({"Mois": mois_noms[m-1], "CA": ca_m, "Frais": fr_m, "Net": ca_m - fr_m})
    ...
    for s in stats_mois:
        html += f'<tr><td>{s["Mois"]}</td><td>{s["CA"]:.0f}€</td><td>-{s["Frais"]:.0f}€</td><td>{s["Net"]:.0f}€</td></tr>'
    st.markdown(html + '</table>', unsafe_allow_html=True)

elif st.session_state.page == "FRAIS":
    st.markdown('<div class="page-title">🔧 MAINTENANCE</div>', unsafe_allow_html=True)
    if st.button("➕ AJOUTER MAINTENANCE", use_container_width=True): st.session_state.form_frais_open = True; st.rerun()
    # (Affichage des frais ici...)
    if not df_frais.empty:
    ...
    # (Code planning avec détail restauré plus haut...)
    st.write(f"Mois: {st.session_state.cal_month}/{st.session_state.cal_year}")
