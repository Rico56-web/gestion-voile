...
if st.session_state.page == "CONTACTS":
    ...
    if st.button("➕ NOUVEAU CLIENT / NAV", use_container_width=True):
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt').iterrows():
            ...
            # --- ORDRE DEMANDÉ : Nom, Prénom, Tel, Mail, Date, Jours, Société, Prix ---
            st.markdown(f"""
            <div class="client-card" style="border-left: 10px solid {"#3498db" if str(r.get('Société'))=="CMN" else "#ccc"};">
                ...
                🏢 Société : <b>{r.get('Société','')}</b> | 💰 Prix : <b>{r.get('PrixJour','0')} €</b>
            </div>""", unsafe_allow_html=True)
            
            note_val = st.text_area("Notes (S'étend automatiquement)", value=r.get('Notes',''), key=f"nt_{i}", height=70)
            if note_val != r.get('Notes',''):
                df.at[i, 'Notes'] = note_val
                sauvegarder_data(df, "contacts.json")
            
            cb1, cb2, _ = st.columns([1,1,4])
            if cb1.button("✏️", key=f"ed_{i}"): st.session_state.edit_idx=i; st.session_state.page="FORM"; st.rerun()
            if cb2.button("🗑️", key=f"dl_{i}"): df=df.drop(i); sauvegarder_data(df, "contacts.json"); st.rerun()
    ...
    if not df.empty:
        ...
        for m in range(1, 13):
            ...
            # Sécurité : on vérifie si la colonne Paiement existe, sinon on met "Unpaid"
            if 'Paiement' not in df_m.columns: df_m['Paiement'] = "Unpaid"
            
            prevu = df_m[df_m['Statut'].str.contains("Attente", na=False)]['PrixJour'].apply(to_f).sum()
            df_ok = df_m[(df_m['Statut'].str.contains("OK", na=False)) & (df_m['Paiement'] == "Paid")]
            fait = df_ok['PrixJour'].apply(to_f).sum()
            frais = df_m['Frais'].apply(to_f).sum() if 'Frais' in df_m.columns else 0.0
            
            stats_mois.append({"Mois": m, "Prévu (€)": int(prevu), "Fait (€)": int(fait), "Frais (€)": int(frais), "NET (€)": int(fait - frais)})
