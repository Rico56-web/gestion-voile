...
df, df_f, df_n, df_s = charger_data("contacts.json"), charger_data("frais.json"), charger_data("notes.json"), charger_data("secu.json")
...
if st.session_state.page == "LISTE":
    ...
    if not df.empty:
        ...
        for i, r in data.sort_values('dt').iterrows():
            ...
            st.markdown(f"""<div class="client-card" style="border-left: 10px solid {border_color};"><div class="status-badge" style="color:{badge_color}; border-color:{badge_color}; background:{badge_color}15;">{statut}</div><b style="font-size:1.1rem; color:#1a2a6c;">{r.get('Prénom','')} {r.get('Nom','').upper()}</b><br>🏢 <b>{soc}</b> | 📅 {r.get('DateNav')}<br>📧 <a href="mailto:{r.get('Email','')}">{r.get('Email','')}</a><br>📞 <a href="tel:{tel}">{r.get('Téléphone','')}</a><br><a href="https://wa.me/{tel}" target="_blank" class="wa-btn-discret">💬 WhatsApp</a></div>""", unsafe_allow_html=True)
    ...
    s_y = st.selectbox("Année", [2025, 2026], index=1); obj = st.number_input("Cible €", value=15000)
    ...
    st.write(f"📈 **{ca/obj*100:.1f}%** ({fmt_p(ca)})"); st.progress(min(ca/obj, 1.0))
    st.markdown("---")
    ...
    f_m = st.selectbox("Mois", range(1, 13), index=datetime.now().month-1, format_func=lambda x: calendar.month_name[x])
    ...
    if not df_c.empty:
        ...
        st.markdown(f'<a href="mailto:contact@cmn.fr?subject=Facture&body={urllib.parse.quote(corps)}" style="background-color:#1a2a6c;color:white;padding:12px;display:block;text-align:center;text-decoration:none;border-radius:8px;">📧 ENVOYER</a>', unsafe_allow_html=True)
    ...
    if st.session_state.edit_f_idx is not None:
        ...
        for i in reversed(df_f.index):
            r = df_f.loc[i]; st.markdown(f'<div class="client-card"><b>{r.get("Date")} : {fmt_p(r.get("Montant"))}</b><br>{r.get("Note")}</div>', unsafe_allow_html=True)
