...
# --- CONFIGURATION GITHUB (Indispensable pour Streamlit Cloud) ---
# Assure-toi d'avoir ajouté GITHUB_REPO et GITHUB_TOKEN dans tes secrets Streamlit
...
def charger_notes():
    url = f"https://api.github.com/repos/{REPO}/contents/notes.txt"
    res = requests.get(url, headers={"Authorization": f"token {TOKEN}"})
    if res.status_code == 200:
        return base64.b64decode(res.json()['content']).decode('utf-8'), res.json()['sha']
    return "", None

def sauvegarder_notes(texte, sha):
    url = f"https://api.github.com/repos/{REPO}/contents/notes.txt"
    data = {
        "message": "Mise à jour notes.txt",
        "content": base64.b64encode(texte.encode('utf-8')).decode('utf-8'),
        "sha": sha
    }
    requests.put(url, headers={"Authorization": f"token {TOKEN}"}, json=data)

# --- TA CONFIGURATION ET TON STYLE ---
    ...
    for sem in jours_cal:
        ...
        for i, d in enumerate(sem):
            if d.month == sel_mois:
                ...
                cols[i].markdown(f'''<div style="background:{bg}; border:{bord}; padding:10px 2px; border-radius:5px; text-align:center; min-height:65px;">
                    <div style="font-weight:bold;">{d.day}</div><div style="font-size:9px; color:#555;">{info}</div></div>''', unsafe_allow_html=True)
            else: cols[i].write("")

# --- PAGE NOTES ---
elif st.session_state.page == "NOTES":
    st.title("📝 Bloc-Notes")
    current_notes, sha = charger_notes()
    ...
    if st.button("💾 SAUVEGARDER LES NOTES", use_container_width=True, type="primary"):
        sauvegarder_notes(new_notes, sha)
