...
# =================================================================
# --- 9. PAGE STATS (BILAN DE TRÉSORERIE RÉELLE - ENCAISSÉ) ---
# =================================================================
if st.session_state.page == "STATS":
    ...
    # --- 4. TRAITEMENT DES DONNÉES & FILTRAGE STRICT (NOUVEAU) ---
    t_rev, t_fra = 0, 0
    df_r_y, df_f_y = pd.DataFrame(), pd.DataFrame()

    # A. REVENUS (Planning) - FILTRAGE STRICT SUR "PAYÉ"
    if not df_p_actif.empty:
        df_p = df_p_actif.copy()
        
        # --- FILTRE 1 : UNQUEMENT CE QUI EST PERÇU (PAYÉ) ---
        ...
        if col_paye:
            # On force en majuscule, on nettoie, et on ne garde que si contient "PAYÉ"
            # J'utilise str.contains pour gérer "Payé par CB", "Payé", etc.
            df_p[col_paye] = df_p[col_paye].fillna('').astype(str).str.upper().str.strip()
            # On cherche spécifiquement "PAYÉ" (avec l'accent encodé ou non)
            mask_paye = df_p[col_paye].str.contains('PAY', na=False)
            df_rev_percus = df_p[mask_paye].copy()
        else:
            # Si aucune colonne de paiement n'existe, on ne prend aucune mission (sécurité)
            df_rev_percus = pd.DataFrame(columns=df_p.columns)
            st.error("⚠️ Colonne de statut de paiement non trouvée dans le planning. Impossible de calculer les revenus perçus.")

        if not df_rev_percus.empty:
            # --- FILTRE 2 : DATE ET ANNÉE ---
            # Conversion date
            df_rev_percus['dt_vrai'] = df_rev_percus['DateNav'].apply(conversion_date_robuste)
            
            # Filtre Année
            mask_y = (df_rev_percus['dt_vrai'].dt.year == sel_y) if mode_bilan == "Par Saison" else ((df_rev_percus['dt_vrai'].dt.year == today.year) & (df_rev_percus['dt_vrai'].dt.date <= today))
            # Tri chronologique inverse (plus récent en haut)
            df_r_y = df_rev_percus[mask_y].sort_values('dt_vrai', ascending=False).copy()
            ...
            if not df_r_y.empty:
                ...
                df_r_y['Société'] = df_r_y['Société'].replace({'PARTICULIER': 'PERSO', 'NAN': 'PERSO', '': 'PERSO', 'CLICK': 'CLICK & BOAT', 'CLICK&BOAT': 'CLICK & BOAT', 'CLICK AND BOAT': 'CLICK & BOAT', 'NONE': 'PERSO'})

    # B. FRAIS (Maintenance) - On considère que toute facture entrée est décaissée
    ...
    st.subheader(f"💰 Synthèse Trésorerie réelle {sel_y}")
    
    # Affichage simplifié sans delta car delta par rapport à "prévu" n'a plus de sens ici
    c1, c2, c3 = st.columns(3)
    solde = t_rev - t_fra
    c1.metric("Encaissé (Missions)", f"{t_rev:,.0f} €".replace(',', ' '))
    ...
    if t_rev > 0 or t_fra > 0:
        ...
        rs_m = []
        for i in range(1, 13):
            r = df_r_y[df_r_y['dt_vrai'].dt.month == i]['P_Num'].sum() if not df_r_y.empty else 0
            f = df_f_y[df_f_y['dt_vrai'].dt.month == i]['M_Num'].sum() if not df_f_y.empty else 0
            # J'arrondis à l'euro pour iPhone
    ...
    # --- 7. DÉTAIL DES OPÉRATIONS DE TRÉSORERIE ---
    st.divider()
    ...
    with st.expander("📥 Détail des Revenus Encaissés", expanded=False):
        if not df_r_y.empty:
            ...
            cols_to_show = [c for c in ['DateNav', 'Client', 'Société', 'Prix'] if c in df_disp.columns]
            # Je renomme "Prix" en "Montant €" pour le tableau
            st.dataframe(df_disp[cols_to_show].rename(columns={'DateNav':'Date','Prix':'Montant €'}), hide_index=True, use_container_width=True)
            st.caption(f"Total Encaissé : **{t_rev:,.0f} €**".replace(',', ' '))
        else:
    ...
    if mode_bilan == "Par Saison":
        ...
        if st.checkbox("Afficher les outils d'archivage", key="stats_arch_check"):
            ...
            st.warning(f"Attention, l'archivage va archiver **toutes** les données de l'année **{sel_y}**, perçues ou non.")
            # ... (Logique d'archivage sécurisée identique à la précédente)
