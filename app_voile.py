...
if st.session_state.page == "CONTACTS":
    ...
    if st.session_state.get('edit_idx') is not None:
        ...
        with st.expander(f"📝 MODIFIER : {r.get('Prénom','')} {r.get('Nom','')}", expanded=True):
            with st.form("form_unique_edit"): # Clé de formulaire unique
                c1, c2 = st.columns(2)
                ...
                u_statut = c_st.selectbox("Statut", ["En attente", "OK", "Refusé", "Terminé"], index=0)
                u_paye = c_pa.selectbox("Paiement", ["Unpaid", "Paid"], index=0)
                ...
                if st.form_submit_button("💾 ENREGISTRER LES MODIFICATIONS"):
                    ...
                    st.session_state.edit_idx = None
                    st.rerun()
        if st.button("❌ Annuler la modification", key="cancel_edit_top"):
            st.session_state.edit_idx = None
            st.rerun()
    ...
    if n1.button("📂 En Cours", key="nav_encours"): st.session_state.view_archive = False
    if n2.button("🗄️ Archives", key="nav_archive"): st.session_state.view_archive = True
    if n3.button("➕ Ajouter", key="nav_add"):
        new_row = {"Prénom": "Nouveau", "Nom": "Contact", "Société": "PARTICULIER", "Statut": "En attente"}
    ...
    for i, r in df_disp.iterrows():
        ...
        <div style="border: 2px solid #1a2a6c; border-radius: 10px; padding: 15px; margin-bottom: 10px; background-color: white; color: black;">
            <div style="display: flex; justify-content: space-between;">
                <b style="color: #1a2a6c;">{r.get('Prénom','')} {r.get('Nom','').upper()}</b>
                <span style="background:{s_col}; color:white; padding:2px 5px; border-radius:5px; font-size:0.7rem;">{s}</span>
            </div>
            <div style="color: #666; font-size: 0.8rem;">🏢 {str(r.get('Société','')).upper()}</div>
            <div style="margin-top: 10px; font-size: 0.9rem; color: black;">
                📞 <b>{tel if tel else "Non renseigné"}</b><br>
                ✉️ <b>{mail if mail else "Non renseigné"}</b>
            </div>
            <div style="margin-top: 10px; display: flex; gap: 5px;">
                <a href="tel:{tel}" style="flex:1; background:#3498db; color:white; padding:8px; border-radius:5px; text-decoration:none; text-align:center; font-size:0.7rem;">APPEL</a>
                <a href="mailto:{mail}" style="flex:1; background:#e67e22; color:white; padding:8px; border-radius:5px; text-decoration:none; text-align:center; font-size:0.7rem;">MAIL</a>
            </div>
        </div>
        '''
        st.markdown(html_fiche, unsafe_allow_html=True)

        # BOUTONS ACTIONS (C'est ici qu'on met des clés uniques)
        c_ed, c_del = st.columns([1, 4])
        if c_ed.button("✏️", key=f"btn_edit_list_{i}"): # Clé unique !
            st.session_state.edit_idx = i
            st.rerun()
        if c_del.button("🗑️ SUPPRIMER", key=f"btn_del_list_{i}"): # Clé unique !
