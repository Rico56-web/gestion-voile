...
if not st.session_state.authenticated:
    ...
    # AJOUT DE LA COLONNE 'Jours' DANS LA STRUCTURE
    ...
    if c1.button("📅 LISTE DES NAVS", use_container_width=True):
        st.session_state.page = "LISTE"
        st.rerun()
    ...
    if st.session_state.page == "LISTE":
        ...
        if filt_df.empty:
            ...
            for idx, row in filt_df.iterrows():
                ...
                # Affichage de la durée (Jours) dans le bandeau
                ...
                with b4:
                    with st.expander("Détails"):
                        st.write(f"⏳ **Durée demandée :** {row['Jours']} jour(s)")
    ...
    # --- PAGE FORMULAIRE ---
        ...
        with st.form("form_nav"):
            ...
            with c_l:
                f_date = st.text_input("Date de début (AAAA-MM-JJ)", value=init.get("DateNav", ""))
                # NOUVEAU CHAMP NOMBRE DE JOURS
                f_jours = st.number_input("Nombre de jours demandés", min_value=0, value=int(init.get("Jours", 0)) if init.get("Jours") else 0)
            ...
            with c_r:
                ...
                f_cau = st.text_input("Motif / Cause du statut", value=init.get("Cause", ""))
            
            st.markdown("---")
            c_tel, c_ema = st.columns(2)
            with c_tel:
                f_tel = st.text_input("Téléphone", value=init.get("Téléphone", ""))
            with c_ema:
                f_ema = st.text_input("Email", value=init.get("Email", ""))
                
            f_dem = st.text_area("Précisions sur la demande", value=init.get("Demande", ""))
            f_his = st.text_area("Historique / Notes Skipper", value=init.get("Historique", ""))
            ...
            if st.form_submit_button("💾 ENREGISTRER LA NAVIGATION"):
                new_row = {
                    "DateNav": f_date, 
                    "Jours": str(f_jours), 
                    "Statut": f_stat, 
