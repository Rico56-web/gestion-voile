...
for i, name in enumerate(menu):
    if cols_nav[i].button(icones[name], key=f"nav_{name}", use_container_width=True, 
        ...
        st.rerun()
# =================================================================
# --- 5. BLOC CONTACTS (V49 - TRI CHRONO FORCÉ & VISIBILITÉ) ---
# =================================================================
if st.session_state.page == "CONTACTS":
    st.markdown('<div style="text-align:center; background-color:#ecf0f1; padding:10px; border-radius:10px;"><h2>👤 Gestion des Contacts</h2></div>', unsafe_allow_html=True)
    ...
    st.session_state.filtre_annee = annee_sel

    if not df_raw.empty:
        df_c = df_raw.copy()
        df_c['orig_idx'] = df_c.index 
        
        # --- LOGIQUE DE TRI CHRONOLOGIQUE INVERSÉ ---
        # On s'assure que la colonne DateNav est bien lue comme une date
        df_c['dt_sort'] = pd.to_datetime(df_c['DateNav'], errors='coerce')
        # Tri : False pour descendant (Plus récent -> Plus ancien)
        df_c = df_c.sort_values(by='dt_sort', ascending=False)

        # Filtre par Année sélectionnée
        df_c = df_c[df_c['dt_sort'].dt.year == annee_sel]

        # Filtre Recherche (Nom ou Société)
        if search:
            mask = df_c['Nom'].str.upper().str.contains(search, na=False) | \
                   df_c['Prénom'].str.upper().str.contains(search, na=False) | \
                   df_c['Société'].str.upper().str.contains(search, na=False)
            df_c = df_c[mask]

        # --- SÉPARATION EN COURS / ARCHIVES ---
        # "En cours" = tout ce qui n'est pas Terminé, Refusé ou Annulé
        list_archives = ["terminé", "refusé", "annulé", "termine", "refuse", "annule"]
        mask_arch = df_c['Statut'].str.lower().isin(list_archives)
        ...
        if n3.button("➕ NOUVEAU", use_container_width=True):
            new_r = {"Prénom": "Nouveau", "Nom": "Contact", "Statut": "En attente", "Paiement": "Non payé", "DateNav": f"{annee_sel}-06-01", "Société": "PERSO", "Prix": 0, "Acompte": 0, "Nbre de personnes": 1, "Nbre de jours": 1}
        ...
        for _, row in df_aff.iterrows():
            ...
            # --- VISUEL CARTE ---
            statut = str(row.get('Statut', 'En attente')).lower()
            ...
            html_card = f"""
            <div style="background-color:{bg}; color:#2C3E50; padding:15px; border-radius:12px; font-family:sans-serif; border:1px solid rgba(0,0,0,0.05); margin-bottom:5px;">
                <div style="display:flex; justify-content:space-between; border-bottom:1px solid rgba(0,0,0,0.1); padding-bottom:5px;">
                    <span style="font-size:1.1rem; font-weight:bold;">{str(row.get('Prénom','')).upper()} {str(row.get('Nom','')).upper()}</span>
                    <span style="font-size:0.75rem; background:white; padding:2px 6px; border-radius:4px; font-weight:bold;">{soc}</span>
                </div>
                <div style="margin-top:10px; display:grid; grid-template-columns: 1fr 1fr; font-size:0.85rem; gap:5px;">
                    ...
                    <div>💰 Prix : <b>{p_tot}€</b></div>
                    <div>💳 Reste : <b style="color:#C0392B;">{p_tot - p_aco}€</b></div>
                </div>
                <div style="margin-top:10px; display:flex; gap:8px;">
                    <a href="tel:{tel}" style="flex:1; text-decoration:none; background:#5DADE2; color:white; padding:10px; border-radius:8px; text-align:center; font-weight:bold; font-size:0.8rem;">APPEL</a>
                    <a href="https://wa.me/{tel.replace(' ','')}" style="flex:1; text-decoration:none; background:#52BE80; color:white; padding:10px; border-radius:8px; text-align:center; font-weight:bold; font-size:0.8rem;">WA</a>
                    <a href="mailto:{mail}" style="flex:1; text-decoration:none; background:#EC7063; color:white; padding:10px; border-radius:8px; text-align:center; font-weight:bold; font-size:0.8rem;">MAIL</a>
                </div>
            </div>
            """
            ...
            if st.session_state.confirm_del_idx == idx:
                col_y, col_n = c_del.columns(2)
                if col_y.button("✅", key=f"y_{idx}", use_container_width=True):
                    df_raw = df_raw.drop(idx)
                    sauvegarder_data(df_raw, 'contacts.json')
                    st.session_state.confirm_del_idx = None; st.rerun()
                if col_n.button("❌", key=f"n_{idx}", use_container_width=True):
                    st.session_state.confirm_del_idx = None; st.rerun()
            else:
